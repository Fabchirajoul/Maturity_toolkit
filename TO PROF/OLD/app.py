...
def submit_code():
    ...
    if request.method == 'POST':
        ...
        # querry to select the data to be used in displaying the diamong plot
        cursor.execute('''
        ...
        # THis section is for the affinity diamond plot display
        ...
        for business_function, data in business_functions_data.items():
            ...
            # Ensure the plot is closed by repeating the first point
            angles += angles[:1]

            plt.figure(figsize=(6, 6))
            ax = plt.subplot(111, polar=True)
            ax.plot(angles, as_is + [as_is[0]], 'o-',
                    linewidth=2, label='AS IS', color='red')
            ax.fill(angles, as_is + [as_is[0]], alpha=0.25, color='red')
            ax.plot(angles, to_be + [to_be[0]], 'o-',
                    linewidth=2, label='TO BE', color='blue')
            ax.fill(angles, to_be + [to_be[0]], alpha=0.25, color='blue')
    ...
    return render_template('userAccount.html', user_records=user_records, percentages=percentage_values,
                           ...
                           error_message=error_message, business_function_map=submission_affinities_data, affinity_record=affinity_record_sorted, plot_images=plot_images)
